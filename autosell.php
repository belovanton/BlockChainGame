...
$autobuildoperations_range="'Рынок Машин'!H50:M380";
$autoselloperations_range="'Рынок Машин'!O50:W380";
$market_fact_range="'Рынок Машин'!E4:H6";
$market_size_range="'Рынок Машин'!A4:D6";
...
function sell($autobuildoperation, $service, $spreadsheetId, $stock_cars_range, $market_fact_range, $market_size_range){
  $check=false;
  $message="";
  $result = $service->spreadsheets_values->get($spreadsheetId, $stock_cars_range);
  ...
  if ($row[1]==$autobuildoperation[2]){
    if($autobuildoperation[4]<=$row[$autobuildoperation[0]+1] ){
      ...
      $check=false;
      $message="Недостаточно машин на складе";
    }
  }
...
//Копируем транзакции продаджи машин
...
foreach($values as $row){
  ...
  if($row[2]=="Продать на рынок" && $row[15]=="Машины" && $row[28]=="Одобрено")
  {
    //print_r($row); die;
    //print_r($row); die;
    ...
    $autoselloperation[6]=sell($autoselloperation, $service, $spreadsheetId, $stock_cars_range, $market_fact_range, $market_size_range);
